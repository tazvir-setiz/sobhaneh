...
1. **احراز هویت (authenticate):** خط اول را می‌خواند، باید فرمت `connect <token>`
   ...
   کار جداگانه چک شود — فقط کافی است شناسهٔ کاربر از `whois` معتبر باشد. اگر معتبر
   بود یک `UserSession` می‌سازد.
...
- اگر هنوز ندارد، اطمینان حاصل کنید که یک اتریبیوت **نام فضای کار** (رشته‌ای که
  موقع ساخت این فضای کار، از دستور `create-workspace <port> <userId>` که مرکزی
  به میزبان می‌فرستد گرفته شده) به‌عنوان فیلد این کلاس نگه‌داری می‌شود — چون بعداً
  در پیام‌های خطا، لاگ‌ها، نام فایل‌های ذخیره‌سازی (`HostDataStore`، روز ۵)، و هر
  جای دیگری که لازم است بدانید این `Workspace` دقیقاً کدام است، به آن نیاز
  خواهید داشت. یک متد getter ساده برای خواندن این نام کافی است.
